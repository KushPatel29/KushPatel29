...
**12,520 automated tests across the sixteen featured repos below**. A green badge here means *it runs*, not
...
| ⚡ [**Utility FP&A in Excel and Power BI**](https://github.com/KushPatel29/excel-fpa-model) · [**workbook ↓**](https://github.com/KushPatel29/excel-fpa-model/raw/main/workbook/PGE_Utility_FPA_Model.xlsx) · [**board pack**](https://github.com/KushPatel29/excel-fpa-model/blob/main/docs/board_pack.pdf) · [**Power BI**](https://github.com/KushPatel29/excel-fpa-model#the-same-numbers-in-power-bi) | Portland General Electric, modelled from **public filings**: EIA monthly sales by customer class, twelve years of audited FERC Form 1 statements and NOAA degree days. A plan built only from 2025 data missed the first half of 2026 by **$100.9M**, and the variance bridge splits it to the dollar: **weather −$40.4M** (a LINEST model, R² 0.84, normalizes load), **price −$42.3M**, usage −$18.3M. Price-volume-mix shows 2019–2025 growth was $994M of price against $275M of volume. A four-year backtest is the honest part: load is forecastable to about 4%, price is not, so a real plan takes price from the rate-case calendar. EIA and FERC are reconciled to each other, and an independent pandas model holds every figure to the cent. The same analysis is a seven-page **Power BI report** generated as PBIP/TMDL, with rate and load what-if sliders, and a test holds its tables to the workbook's own cells, so the two cannot disagree. 1,180 tests. | Excel, Power Query, Power Pivot, DAX, LINEST, LAMBDA, what-if data tables, Power BI (PBIP/TMDL) |
