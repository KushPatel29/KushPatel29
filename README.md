...
| 🏪 [**Retail Analytics Platform**](https://github.com/KushPatel29/wholesale-analytics-platform) · [**live ▶**](https://kushpatel29.github.io/wholesale-analytics-platform/) | A sixteen-page BI platform that once showed **three different HHI values on three pages at the same time**, because every page did its own arithmetic. Now forty-two metrics live in one layer, and each carries a catalogue entry naming its formula, grain, source table and *gross-or-net basis* — including the ones the demand planner never had, like scoring its own forecast against actuals. Metrics that would need a source system that doesn't exist are listed as **not implemented, with the system each would require**, rather than filled in with a plausible number. Published as a prerendered static site: every chart frozen to SVG at build time, so all sixteen pages render with networking blocked. 1,194 tests. | Python, Flask, DuckDB, Plotly, static site generation |
