...
and re-verifies its own claims on every push — **8,887 automated tests across
the thirteen featured repos below**. One additional CI-backed utility — the
[inventory analytics app](https://github.com/KushPatel29/inventory-analytics-app)
— brings the public-repo total to fourteen without changing that featured-set
test total. A green badge here means *it runs*, not just that it's written down.

Six of them have **hosted endpoints you can click** — no local install:
