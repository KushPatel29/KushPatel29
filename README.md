...
Analytics runs on Render; the other nine interactive destinations run on
...
| 🔥 [**Canada Wildfire Risk**](https://github.com/KushPatel29/wildfire-prediction-app) | The one project here that runs on real data, and the one I have been beaten by. It began as a 2024 hackathon win — a random forest on hectares burned per month — and it is now a daily seven-day ignition forecast for Canada's 771 one-degree cells, over 4.1M cell-days of the National Fire Database and 3,198 CWFIS stations. The Fire Weather Index is implemented from Van Wagner and replayed against CWFIS's own published codes before anything leans on it. Two boosters (a classifier and a Poisson count) are averaged and isotonically calibrated, trained to 2016, tuned on 2017–2019, and scored **once** on 2020–2024: **ROC-AUC 0.807** against 0.780 for each cell's own rate for the month, with **36.7% of 26,975 fires inside the riskiest tenth re-ranked every morning**. Then the part I would read first: graded against this season's satellite detections, **the Fire Weather Index alone beats the any-fire model** — 0.719 to 0.692 — because a satellite sees the fires weather drives and the model is trained on every reported start. Both numbers are on the page. The old random forest is rebuilt too, and its R² falls from 0.351 on a random month split to 0.201 out of time. GitHub Actions rebuilds the forecast twice a day; an eight-page Power BI project is generated from spec. 108 tests. | Python, XGBoost, FWI System, GIS, Power BI, GitHub Actions |
